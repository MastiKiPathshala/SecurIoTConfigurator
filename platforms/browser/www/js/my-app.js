...
function startScan() {
    $('#statsDiv').html('Scanning for Gateways');
...
function checkLoop() {
    ...
    if ($("#homeScreenCounter").data('seconds') > 180 && $("#rasbList li").length <= 0 && mainView.activePage.name == 'index') {
        gatewayPopup('Please turn on or restart the Prophecy Gateway')
        ...
        for (var j = 0; j < raspSize; j++) {
            ...
            if (data[i].ssid == selectedWifi[j]) {
                ...
                item = '<div class="card"  ' + bgStyl + ' >' +
                    ...
                    '<div class="item-inner">' +
                    '<div class="item-title label">Wi-Fi Password</div>' +
                ...
                $.ajax({
                    ...
                    if (res.status) {
                        ...
                        if (bypass) {
                            ...
                            myApp.alert('Please plug in the Ethernet cable', '<img height="150px" src="img/ethernet_cable.png">')

                        }

                        if (RPIENDPOINT != 'http://10.0.0.1') {
                            myApp.alert('Please unplug the usb cable now')
                        }
    ...
    if (typeof interfaces == "undefined" || interfaces == "" || !interfaces) {
        ...
        if (interfaces.length > 0) {
            for (var x in interfaces) {
                ...
                if (interfaces[x] != "" || typeof interfaces[x] != "undefined" || interfaces[x] != null) {

                    if (interfaces[x].indexOf('eth') > -1) {


                        var imgTag = '<img src="img/ethernet.png" height="50" >';
                        var intrFace = "Ethernet (wired)";

                        var item = '<li><a href="#" onclick=\'chooseInterface(\"' + interfaces[x] + '\",this)\' class="item-content"><div class="item-inner">' +
                            imgTag + '  ' + intrFace + '</div></a></li>';
                        $("#intrfaceLst").append(item);


                    } else if (interfaces[x].indexOf('wlan') > -1) {
...
function startWifigurator(element) {
    ...
    if (md.os() == 'AndroidOS') {
        ...
        if (RPIENDPOINT == "http://192.168.42.42") {
            ...
            myApp.modal({
                title: 'Join data hub',
                text: '<ol type="1"><li>Open Wi-Fi Settings</li><li>Wait for Prophecy data hub</li><li>Choose the Prophecy data hub</li><li>Enter password "raspberry"</li><li>Switch back to this App</li></ol>',
                buttons: [{
...
function getRpiEndpoint() {
    ...
    RPIENDPOINT = window.localStorage.getItem('RPIURL') || 'http://10.0.0.1'
