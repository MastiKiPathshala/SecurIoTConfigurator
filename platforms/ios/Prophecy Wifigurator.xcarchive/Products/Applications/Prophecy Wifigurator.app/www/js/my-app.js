            ...
            if (isConnected) {
                ...
                $.ajax({
                    ...
                    if (res.status) {
                        ...
                        if (RPIENDPOINT != 'http://10.0.0.1') {
                            myApp.alert('Please unplug the usb cable now, and connect this device to the internet')
                        }
...
function appendHidden() {
   ...
   item = '<label class="label-checkbox item-content">' 
   + '<input type="checkbox" name="wifiList[]"  value="' 
   + data[i].ssid + '" >' + '<div class="item-media"> <i class="icon icon-form-checkbox" >'
   +' </i></div><div class="item-inner"><div class="item-title" style="white-space: normal !important"> SSID: ' 
   + data[i].ssid + ' (' + data[i].security + ') </div></div></label>'
